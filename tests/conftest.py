...
@pytest.fixture
def synthetic_raw_data(synthetic_price_df, synthetic_funding_df, synthetic_oi_df) -> dict[str, pd.DataFrame]:
    ...
    return {
        ...
        "funding": pd.DataFrame({
            "timestamp": ts, "close": synthetic_funding_df["funding_close"].values,
            "open": synthetic_funding_df["funding_close"].values * 0.99,
            "symbol": "BTC", "exchange": "Binance",
        }),
        "oi": pd.DataFrame({
            "timestamp": ts, "close": synthetic_oi_df["oi_close"].values,
