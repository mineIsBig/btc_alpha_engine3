...
class ConsensusGate:
    ...
    def __init__(
        self,
        min_horizon_agreement: int | None = None,
        ...
        self.min_horizon_agreement = (
            min_horizon_agreement if min_horizon_agreement is not None else 2
        )
    ...
    def check(self, horizon_signals: dict[int, AggregatedSignal]) -> tuple[int, str]:
        ...
        if (
            len(long_horizons) >= self.min_horizon_agreement
            and len(short_horizons) < self.min_horizon_agreement
        ):
            return 1, f"long_consensus_horizons={long_horizons}"
        elif (
            len(short_horizons) >= self.min_horizon_agreement
            and len(long_horizons) < self.min_horizon_agreement
        ):
            ...
            return (
                0,
                f"conflicting_consensus long={long_horizons} short={short_horizons}",
            )
        else:
            return (
                0,
                f"insufficient_agreement long={len(long_horizons)} short={len(short_horizons)}",
            )
